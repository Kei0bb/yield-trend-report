    ...
    return pd.DataFrame([
        {
            ...
            "item_name": "Vtl_ITEM", "item_unit": "V",
...
def test_lot_series_is_ordered_oldest_first():
    ...
    series = build_lot_series(df, "Vtl_ITEM", 0.0, 1.0, _lot_dates(df))
...
def test_period_stats_pool_raw_measurements_not_lot_means():
    ...
    core = _item_core(df, "Vtl_ITEM", 0.0, 1.0)
    assert core["n"] == 4
...
def test_period_spec_conflict_takes_the_most_common_and_warns(caplog):
    ...
    with caplog.at_level(logging.WARNING):
        assert resolve_spec(df["spec_high"], "Vtl_ITEM") == 1.0
...
def test_get_wat_trend_reports_the_mock_products_items_and_lots():
    ...
    assert [i.item_name for i in res.items] == sorted(
        (i.item_name for i in res.items), key=item_sort_key)
...
def test_get_wat_trend_still_flags_the_deliberate_mock_defects():
    ...
    assert by_name["Vtl_N_ULVT"].status == "red"
